...
for i=2:final/step+1
    if(rem(i,3)==0)
        if(transmitted(i-1)<msg(i))
            deltaPlus = deltaPlus+delta;
            deltaMinus = delta;
            delta_train(i) = deltaPlus;
            transmitted(i) = transmitted(i-1)+deltaPlus;
        else
            deltaPlus = delta;
            deltaMinus = deltaMinus+delta;
            delta_train(i) = -deltaMinus;
            transmitted(i) = transmitted(i-1)-deltaMinus;
        end
    else
